...
export const getLeaderboardFromDb = async (): Promise<Leaderboard[]> => {
  ...
  const result = await query<Leaderboard>(
    "select p.player_name as Player, a.score as Score, to_char(a.time_started, 'Mon DD, YYYY') as Date, to_char(a.time_started, 'HH24:MI') as UTC_Time from attempts a, players p where a.player_id = p.id order by a.score desc limit 10"
  );
  return result.rows;
};
