...
export const getLeaderboardFromDb = async (): Promise<Leaderboard[]> => {
  ...
  const result = await query<Leaderboard>(
    "SELECT DISTINCT ON (p.id) p.player_name AS Player, a.score AS Score, TO_CHAR(a.time_started, 'Mon DD, YYYY') AS Date, TO_CHAR(a.time_started, 'HH24:MI') AS UTC_Time FROM attempts a JOIN players p ON a.player_id = p.id ORDER BY p.id, a.score DESC LIMIT 10"
  );
  return result.rows;
};
